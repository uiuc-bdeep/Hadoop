...
class Feature:
    ...
    def __init__(self, shapeEditor, index):
        ...
        self.parts = shapeEditor.shape(index).parts
    ...
    def does_contain_points(self, x, y):
        ...
        return point_in_poly(x, y, self.p[self.parts[-1]:])
    ...
    def __getitem__(self, ind):
        if is_string(ind):
            m = map(lambda f: f[0], self.shp.fields)
            p = m.index(ind) - 1
            return self.shp.records[self.id][p]
        else:
            return self.shp.shape(self.id).points[ind]

    def __setitem__(self, ind, value):
        if is_string(ind):
            m = map(lambda f: f[0], self.shp.fields)
            p = m.index(ind) - 1
            self.shp.records[self.id][p] = value
            self.__refreshR()
        else:
            self.shp.shape(self.id).points[ind] = value
    ...
    def next(self):
        if self.nowIndex >= self.lens - 1:
            ...
            self.nowIndex += 1
            return self.shp.shape(self.id).points[self.nowIndex]
...
class Editor(Writer):
    ...
    def index_of_nn_feature(self,x, y):
        if hasattr(self, 'root'):
            #print self.treeQuery(x, y, self.root)
            return self.treeQuery(x, y, self.root)
        try:
            print 'try'
            return 0
    ...
    def get_closest_polygon(self, mPolys,x,y):
        result =[]
        for mID in mPolys:
            pts = self[mID].p
            result.append(min(map(lambda i: dist_ponit_to_seg([x,y],pts[i],pts[i+1]),range(len(pts)-1))))
        if min(result) > 165000:
            return -1
        else:
            return mPolys[result.index(min(result))]
        #xx = mPolys[result.index(min(result))]
        #print self.records[xx][6]
        #return xx
    ...
    def birth(self,node):
        ...
        for i in range(4):            
            ...
            newshps=filter(lambda k: is_poly_intersect(newbbox,self.shape(k).points,self.shape(k).bbox), node[0])
            node[2].append(self.birth([newshps,newbbox,[],node[-1]+1]))
        #if node[-1]<6:
        #    print node[-1], node[1]
        return node
    ...
    def treeQuery(self,x,y,node):
        ...
        if choice == 1:
            #print node
            return self.get_closest_polygon(node[0],x,y) 
            ##node[0][0] #if self[node[0][0]].does_contain_points(x,y) else -1
