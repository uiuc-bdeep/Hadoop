...
class Feature:
    ...
    def DPSimplify(self, epsilon):
        pp = list(self.parts) + []
        self.shp.points, self.shp.parts=DPSimplify(self.p, epsilon, self.parts)

    def area(self):
	    """
	    the area of a polygon
	    """
	    return areaOf(self.p,self.parts)

    def length(self):
        return lengthOf(self.p, self.parts)
...
class Editor(Writer):
    ...
    def treeClosest(self, x, y, node):
        ...
        if choice < 100:
            return sorted([(distp2p(self.shape(k).points[0],[x,y]),k) for k in node[0]])[0]#[1] ## FIXING POINT
